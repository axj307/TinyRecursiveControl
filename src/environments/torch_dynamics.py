...
def simulate_pendulum_torch(
    ...
    """
    Simulate pendulum dynamics in PyTorch with differentiable angle wrapping.

    Dynamics:
        I*θ'' = -m*g*l*sin(θ) - b*θ' + u

    State equations:
        dθ/dt = θ'
        dθ'/dt = (1/I) * (-m*g*l*sin(θ) - b*θ' + u)

    Uses Euler integration and atan2-based angle wrapping to maintain θ ∈ [-π, π]
    with smooth, differentiable gradients.

    Args:
        initial_state: Initial state [batch_size, 2] or [2] - [angle, angular_velocity]
        controls: Control sequence [batch_size, horizon, 1] or [horizon, 1] - [torque]
        m: Mass (kg, default: 1.0)
        l: Length (m, default: 1.0)
        g: Gravity (m/s^2, default: 9.81)
        b: Friction coefficient (default: 0.1)
        I: Moment of inertia (kg*m^2, default: m*l^2)
        dt: Time step (default: 0.05s)

    Returns:
        states: State trajectory [batch_size, horizon+1, 2] or [horizon+1, 2]

    Note on Angle Wrapping:
        Uses torch.atan2(sin(θ), cos(θ)) instead of modulo operation to ensure
        smooth gradients at the wrapping boundary. This is critical for gradient-
        based optimization.

    Example:
        >>> initial = torch.tensor([[3.0, 0.0]])  # Start near upside down
        >>> controls = torch.zeros(1, 50, 1)      # No torque
        >>> states = simulate_pendulum_torch(initial, controls, dt=0.05)
        >>> # Pendulum will swing down under gravity
    """
    # Handle both batched and single trajectories
    if len(initial_state.shape) == 1:
        initial_state = initial_state.unsqueeze(0)
    ...
    # Euler integration with differentiable angle wrapping
    for t in range(horizon):
        # Clone to avoid in-place operation issues
        theta = states[:, t, 0].clone()
        theta_dot = states[:, t, 1].clone()
        torque = controls[:, t, 0]

        # Compute angular acceleration
        # θ'' = (1/I) * (-m*g*l*sin(θ) - b*θ' + u)
        theta_ddot = (-m * g * l * torch.sin(theta) - b * theta_dot + torque) / I

        # Euler integration
        new_theta_dot = theta_dot + theta_ddot * dt
        new_theta = theta + new_theta_dot * dt

        # Differentiable angle wrapping to [-π, π]
        # atan2(sin(θ), cos(θ)) is smooth and preserves gradients
        new_theta = torch.atan2(torch.sin(new_theta), torch.cos(new_theta))

        states[:, t+1, 0] = new_theta
        states[:, t+1, 1] = new_theta_dot
